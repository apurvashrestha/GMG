...
router.get('/findCondoUnitFromAID2/AID2/:AID_ID', async(req, res) => {
    var AID = req.params.AID_ID
    try {
        if (!AID) {
            log.debug('Condo nits not found')
        ...
        var requestOptions = {
            'method': "GET",
            'url': `https://citizenatlas.dc.gov/newwebservices/locationverifier.asmx/FindCondoUnitFromAID2?AID=${AID}&f=json`,
            'headers': {
